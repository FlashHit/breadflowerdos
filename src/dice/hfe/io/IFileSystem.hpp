...
namespace dice::hfe::io
{
	...
	constexpr uint32_t IID_IFileSystem = 0x501611FE;
	...
	public:
		...
		virtual IStream* open(const std::string&, FileMode, FileAccess, uint32_t) = 0;
		...
		virtual bool readFile(const std::string&, void*, uint32_t, uint32_t&) = 0;
		virtual bool writeFile(const std::string&, const void*, uint32_t, bool) = 0;
		virtual uint32_t findFiles(const std::string&, bool, std::list<FileInfo>&) = 0;
		virtual bool getFileInfo(const std::string&, FileInfo&) = 0;
		virtual bool compareFileTime(const std::string&, const std::string&) = 0;
